...
class PinCode extends React.PureComponent<IProps, IState> {
      ...
      <View style={[styles.topViewCirclePassword, this.props.styleCircleHiddenPassword]}>
        {_.range(this.props.passwordLength).map((val:number) => {
          const lengthSup =
            ...
            !attemptFailed;
          return (
            <Animate key={val} show={true} start={{
              opacity:0.5,
  ...
  renderButtonDelete = (opacity:number) => {
    return (
      <TouchableHighlight activeOpacity={1} disabled={this.state.password.length === 0} underlayColor="transparent" onHideUnderlay={() =>
        this.setState({
          colorDelete:this.props.styleDeleteButtonColorHideUnderlay
        })
      } onShowUnderlay={() =>
          ...
          {this.props.customBackSpaceIcon ?
            ...
            <>
              {!this.props.iconButtonDeleteDisabled && (
                <Icon name={this.props.styleDeleteButtonIcon} size={this.props.styleDeleteButtonSize} color={this.state.colorDelete} style={{opacity:opacity}}/>
              )}
              <Text style={[
